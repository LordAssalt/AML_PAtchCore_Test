...
class PatchCore(torch.nn.Module)
    def __init__(
        ...
        # Setup backbone net
        print(fVanilla Mode {vanilla})
        print(fNet Used {backbone})

        if vanilla==True
            self.model = torch.hub.load('pytorchvision', 'wide_resnet50_2', pretrained=True)
    ...
    def predict(self, sample tensor)
        ...
        w = 1 - (torch.exp(s_starnorm)  torch.sum(torch.exp(w_denominatornorm)))  # Equation 7 from the paper
        s = w  s_star 
